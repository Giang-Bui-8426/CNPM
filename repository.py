...
admin123,123456,Admin,admin,admin@ut.edu.vn,,admin, """)
    ...
    def save_students(self):
        ...
        writeCsv(pathStudents, rows, ['mssv','password','fullname','role','email','address','major','completedCourse','debtID'])
    ...
    def save_courses(self):
        ...
        writeCsv(pathCourses, rows, ['courseID','courseName','credit','prereq'])
    ...
    def save_schedules(self):
        ...
        writeCsv(pathSchedules, rows, ['scheduleID','dateStart','dateEnd','session','dayOfWeek'])
    ...
    def save_classes(self):
        ...
        writeCsv(pathClasses, rows, ['classID','courseID','roomID','scheduleID','maxStudents','status'])
    def save_regs(self):
        rows = [{'registrationID':r.registrationID,'mssv':r.mssv,'classID':r.classID} for r in self.regs if r.status=='active']
        writeCsv(pathRegs, rows, ['registrationID','mssv','classID','status'])

    def save_debts(self):
        rows = [{'debtID':d.debtID,'debt':d.debt} for d in self.debts.values()]
        writeCsv(pathDebts, rows, ['debtID','debt'])
